...
export default function AnalysisInput({ value, onChange, onAnalyze, onClear, isLoading }) {
  ...
  return (
    ...
    >
      {/* Card header */}
      <div className="px-6 pt-5 pb-4 border-b border-[#f5f3ef]">
        <div className="flex items-center gap-2">
          <Sparkles className="w-4 h-4 text-[#d97757]" />
          <span className="text-sm font-medium text-[#1a1a1a]">Analyze News Content</span>
        </div>
      ...
      <div className="px-6 pt-4 pb-2">
        <textarea
          ...
          rows={4}
          className="w-full resize-none bg-transparent text-[15px] text-[#1a1a1a] placeholder-[#b0a494] outline-none leading-relaxed scrollbar-thin"
      ...
      <div className="px-6 pb-5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={onClear}
            className="flex items-center gap-1.5 text-sm text-[#8a8a8a] hover:text-[#1a1a1a] px-3 py-2.5 rounded-xl hover:bg-[#f5f3ef] transition-colors"
          >
        ...
        <span className="text-xs text-[#b0a494]">
      ...
      <AnimatePresence>
        {!value && (
          <motion.div
            ...
            className="border-t border-[#f5f3ef] px-6 py-4"
          >
            <p className="text-xs text-[#8a8a8a] mb-3 font-medium uppercase tracking-wide">
              Try a sample
            </p>
            <div className="grid grid-cols-2 gap-2">
              {SAMPLES.map((s, i) => (
                <motion.button
                  ...
                  className="text-left p-3 border border-[#ede9e2] rounded-xl transition-colors"
                >
                  <div className="text-xs font-medium text-[#5a5a5a] mb-1">{s.label}</div>
                  <div className="text-xs text-[#8a8a8a] line-clamp-2 leading-relaxed">
                    {s.text}
                  </div>
                </motion.button>
              ))}
